...
void detectAndProcessExceptionDumps(void)
{
    ...
    static const char *handledExceptionNames[] = {
        "FIQ", "istruzioni non definite", "aborto prefetch", "aborto dati"
    },
                      *specialExceptions[] = {
        ...
        "SP", "LR", "PC", "CPSR", "FPEXC"
                      ...
                      *faultStatusNames[] = {
        "Allineamento", "Instr.cache manutenzione op.",
        ...
        "Dominio - Sezione", "Dominio - Pagina", "Permessi - Sezione", "Permessi - Pagina",
    ...
    if(dumpHeader->type == 2)
    {
        if((regs[16] & 0x20) == 0 && dumpHeader->codeDumpSize >= 4)
        {
            ...
            if(instr == 0xE12FFF7E)
                posY = drawFormattedString(true, 10, posY + SPACING_Y, COLOR_WHITE, "Tipo di errore:  %s (%s)", handledExceptionNames[dumpHeader->type], specialExceptions[0]);
            else if(instr == 0xEF00003C)
                posY = drawFormattedString(true, 10, posY + SPACING_Y, COLOR_WHITE, "Tipo di errore:  %s (%s)", handledExceptionNames[dumpHeader->type], specialExceptions[1]);
            else
                posY = drawFormattedString(true, 10, posY + SPACING_Y, COLOR_WHITE, "Tipo di errore:  %s", handledExceptionNames[dumpHeader->type]);
        }
    ...
    if(dumpHeader->processor == 11 && dumpHeader->type >= 2)
    {
        ...
        for(u32 i = 0; i < 15; i++)
            ...
            {
                posY = drawFormattedString(true, 10, posY + SPACING_Y, COLOR_WHITE, "Stato fatalita':    %s", faultStatusNames[i]);
                break;
            }
    }
    ...
    if(dumpHeader->additionalDataSize != 0)
    {
        ...
        if(dumpHeader->processor == 11)
            posY = drawFormattedString(true, 10, posY + SPACING_Y, COLOR_WHITE,
                                    "Processo attuale: %.8s (%016llX)", (const char *)additionalData, *(vu64 *)(additionalData + 8));
        else
            posY = drawFormattedString(true, 10, posY + SPACING_Y, COLOR_WHITE,
                                    "Arm9 memory dump all' offset %X dimensione %lX", (uintptr_t)additionalData - (uintptr_t)dumpHeader, size);
    }
    posY += SPACING_Y;
    ...
    if(dumpHeader->processor == 11 && dumpHeader->type == 3)
        posY = drawFormattedString(true, 10, posY + SPACING_Y, COLOR_WHITE, "%-7s%08lX       Tipo di accesso: %s", "FAR", regs[19], regs[17] & (1u << 11) ? "Write" : "Read");

    posY += SPACING_Y;

    u32 mode = regs[16] & 0xF;
    if(dumpHeader->type == 3 && (mode == 7 || mode == 11))
        posY = drawString(true, 10, posY + SPACING_Y, COLOR_YELLOW, "Dump non corretto: fallito il dump del cosidce e/o dello stack") + SPACING_Y;
    ...
    static const char *choiceMessage[] = {"Premi A per dumpare il crash e salv.", "Premi qualsiasi altro tasto per spegnere"};
    ...
    {
        posY = drawString(true, 10, posY + SPACING_Y, COLOR_WHITE, "Puoi trovare il dump nel seguente file:");
        posY = drawFormattedString(true, 10, posY + SPACING_Y, COLOR_WHITE, "%s:/luma/%s", isSdMode ? "SD" : "CTRNAND", path) + SPACING_Y;
    }
    else posY = drawString(true, 10, posY + SPACING_Y, COLOR_RED, "Errore durante la scrit. del file di dump");

    drawString(true, 10, posY + SPACING_Y, COLOR_WHITE, "Premi un tasto qualsiasi per spegnere");
